...
string_list splitf(const char* str, char delimiter, int first_n) {
	...
	for (size_t i = 0; i <= c_str_len && (!first_n || p->size < first_n); i++) {
		if (str[i] == delimiter || str[i] == '\0') {
			...
			if (!q) {
			malloc_fail:
				p->foreach(p, clear_vstring, NULL);
				delete_vlist(p, &p);
				return NULL;
			}
			p->quick_add(p, q);
			...
			memcpy(q->str, str + de_start_pos, de_end_pos - de_start_pos);
			de_start_pos = de_end_pos + 1;
		}
	}
	return p;
}
...
void delete_string_list(string_list list, string_list* list_addr) {
	if (list)
	{
		list->foreach(list, clear_vstring, NULL);
		delete_vlist(list, list_addr);
	}
}
